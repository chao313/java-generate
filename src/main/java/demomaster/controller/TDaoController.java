...
@RestController
@RequestMapping(value = "/TDaoController")
@Slf4j
public class TDaoController {

    @Autowired
    private TDaoService service;
    ...
    public Response insert(@RequestBody TDaoVo vo) {
    ...
    public Response insert(@RequestBody List<TDaoVo> vos) {
        Response response = new Response();
        try {
    ...
    public Response queryBase(@RequestBody TDaoVo query) {
        Response response = new Response();
        try {
            List<TDaoVo> result = service.queryBase(query);
            response.setCode(Code.System.OK);
            response.setContent(result);
            log.info("success result -> {} ", result);
        } catch (Exception e) {
    ...
    public Response queryBasePageHelper(@RequestBody TDaoVo query,
    ...
    public Response queryMultiTermPageHelper(@RequestBody TDaoMultiTermVo query,
                                             @RequestParam(value = "pageNum", defaultValue = "1", required = false) Integer pageNum,
                                             @RequestParam(value = "pageSize", defaultValue = "10", required = false) Integer pageSize,
                                             @RequestParam(value = "order", required = false) String order) {
         Response response = new Response();
         try {
             ...
             List<TDaoVo> result = service.queryMultiTerm(query);
             PageInfo pageInfo = new PageInfo(result);
             response.setCode(Code.System.OK);
             response.setContent(pageInfo);
             log.info("success pageInfo -> {} ", pageInfo);
    ...
    public Response updateBase(@RequestBody RequestUpdate<TDaoVo,TDaoVo> update) {
        Response response = new Response();
        try {
    ...
    public Response updateBaseIncludeNull(@RequestBody RequestUpdate<TDaoVo,TDaoVo> update) {
        Response response = new Response();
        try {
    ...
    public Response deleteBase(@RequestBody TDaoVo vo) {
        Response response = new Response();
        try {
    ...
    public Response queryByPrimaryKey(
        ...
        Response response = new Response();
        try {
            TDaoVo result = service.queryByPrimaryKey(dbName, dbTableName);
            response.setCode(Code.System.OK);
            response.setContent(result);
            log.info("success result -> {} ", result);
        } catch (Exception e) {
    ...
    public Response updateByPrimaryKey(@RequestBody RequestUpdate<TDaoNoPriVo,TDaoPriVo> update) {
        Response response = new Response();
        try {
